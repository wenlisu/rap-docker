...
import { User, Organization, Repository, Module, Type, Interface, Property, QueryInclude, Logger } from '../models'
  ...
  if (tryCache) {
    ...
    repository = await Repository.findById(ctx.query.id, {
      ...
      order: [
        ...
        [{ model: Module, as: 'modules' }, { model: Type, as: 'types' }, 'priority', 'asc'],
        [{ model: Module, as: 'modules' }, { model: Type, as: 'types' }, { model: Interface, as: 'interfaces' }, 'priority', 'asc']
      ]
    })
...
router.get('/repository/remove', async (ctx, next) => {
  ...
  await Type.destroy({ where: { repositoryId: id } })
...
router.get('/module/remove', async (ctx, next) => {
  ...
  await Type.destroy({ where: { moduleId: id } })
...
// 分类
router.get('/type/count', async (ctx) => {
  ctx.body = {
    data: await Type.count(),
  }
})
...
router.get('/type/get', async (ctx) => {
  ctx.body = {
    data: await Type.findById(ctx.query.id, {
      attributes: { exclude: [] },
    }),
  }
})
router.post('/type/create', async (ctx, next) => {
  let creatorId = ctx.session.id
  let body = Object.assign(ctx.request.body, { creatorId })
  body.priority = Date.now()
  let created = await Type.create(body)
  await initType(created)
  ctx.body = {
    data: await Type.findById(created.id),
  }
  return next()
}, async (ctx) => {
...
router.post('/type/update', async (ctx, next) => {
  const { id, name } = ctx.request.body
  await Type.update({ name, id }, {
    where: { id }
  })
  ctx.body = {
    data: {
...
router.get('/type/remove', async (ctx, next) => {
  ...
  if (ctx.body.data === 0) return
  let { id } = ctx.query
  let type = await Type.findById(id, { paranoid: false })
  await Logger.create({
    userId: ctx.session.id,
    type: 'delete',
    repositoryId: type.repositoryId,
...
router.post('/type/sort', async (ctx) => {
  let { ids } = ctx.request.body
  let counter = 1
  for (let index = 0; index < ids.length; index++) {
    await Type.update({ priority: counter++ }, {
      where: { id: ids[index] }
    })
  }
  if (ids && ids.length) {
    const type = await Type.findById(ids[0])
    await RedisService.delCache(CACHE_KEY.REPOSITORY_GET, type.repositoryId)
  }
  ctx.body = {
    data: ids.length,
  }
...
router.post('/repository/importJson', async (ctx) => {
  if (!ctx.session || !ctx.session.id) {
    ctx.body = {
      isOk: false,
      message: 'NOT LOGIN'
...
router.post('/repository/vsceExportDJson', async (ctx) => {
  if (!ctx.request.body || !ctx.request.body.sessionId) {
    ctx.body = {
      isOk: false,
      message: 'NOT LOGIN'
    }
