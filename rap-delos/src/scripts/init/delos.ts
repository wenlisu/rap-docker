...
import { User, Organization, Repository, Module, Interface, Property, Json, Dto, Enum, Room, Type } from '../../models/index'
import { BO_ADMIN, BO_MOZHI, BO_JSON } from './bo'
import { BO_USER_FN, BO_ORGANIZATION_FN, BO_REPOSITORY_FN, BO_MODULE_FN, BO_INTERFACE_FN, BO_PROPERTY_FN } from './bo'
import { BO_USER_COUNT, BO_ORGANIZATION_COUNT, BO_REPOSITORY_COUNT, BO_MODULE_COUNT, BO_TYPE_FN, BO_TYPE_COUNT, BO_INTERFACE_COUNT, BO_PROPERTY_COUNT } from './bo'
...
export async function init () {
  ...
  await Json.destroy(EMPTY_WHERE)
  await Type.destroy(EMPTY_WHERE)
  await Dto.destroy(EMPTY_WHERE)
  await Enum.destroy(EMPTY_WHERE)
...
async function initRepository (repository: any) {
  ...
  for (let BO_MODULE_INDEX = 0; BO_MODULE_INDEX < BO_MODULE_COUNT; BO_MODULE_INDEX++) {
    ...
    // 接口类型
    for (let BO_TYPE_INDEX = 0; BO_TYPE_INDEX < BO_TYPE_COUNT; BO_TYPE_INDEX++) {
      let type = await Type.create(
        BO_TYPE_FN({ creatorId: repository.creatorId, repositoryId: repository.id, moduleId: mod.id }),
      )
      await mod.$add('types', type)
      // 接口
      for (let BO_INTERFACE_INDEX = 0; BO_INTERFACE_INDEX < BO_INTERFACE_COUNT; BO_INTERFACE_INDEX++) {
        let itf = await Interface.create(
          BO_INTERFACE_FN({ creatorId: mod.creatorId, repositoryId: repository.id, moduleId: mod.id, typeId: type.id }),
        )
        await type.$add('interfaces', itf)
        // 属性
        for (let BO_PROPERTY_INDEX = 0; BO_PROPERTY_INDEX < BO_PROPERTY_COUNT; BO_PROPERTY_INDEX++) {
          let prop = await Property.create(
            BO_PROPERTY_FN({ creatorId: itf.creatorId, repositoryId: repository.id, moduleId: mod.id, typeId: type.id, interfaceId: itf.id }),
          )
