...
class Interface extends Component {
  ...
  render () {
    ...
    let { auth, repository, mod, type, itf } = this.props
        ...
        {isOwned || isJoined
          ? <div className='toolbar'>
            ...
            <RModal when={this.state.update} onClose={e => this.setState({ update: false })} onResolve={this.handleUpdate}>
              <InterfaceForm title='修改接口' repository={repository} mod={mod} type={type} itf={itf} />
            </RModal>
            {!itf.locker
...
class InterfaceList extends Component {
  ...
  render () {
    let { auth, repository, mod, type, itfs = [], itf, curItf } = this.props
    if (!mod.id || !type.id) return null
    let isOwned = repository.owner.id === auth.id
    let isJoined = repository.members.find(itme => itme.id === auth.id)
    return (
          ...
          <ul className='body'>
            {itfs.map(item =>
              <li key={item.id} className={item.id === itf.id ? 'active sortable' : 'sortable'} data-id={item.id}>
                <Interface repository={repository} mod={mod} type={type} itf={item} active={item.id === itf.id} auth={auth} curItf={curItf} />
              </li>
            )}
          </ul>
        </RSortable>
        {isOwned || isJoined
          ? <div className='footer'>
            ...
            <RModal when={this.state.create} onClose={e => this.setState({ create: false })} onResolve={this.handleCreate}>
              <InterfaceForm title='新建接口' repository={repository} mod={mod} type={type} />
            </RModal>
          </div>
          : null}
      </article>
