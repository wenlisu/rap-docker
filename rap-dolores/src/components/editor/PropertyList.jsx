...
class PropertyList extends Component {
  ...
  render () {
    let { title, label, scope, properties = [], repository = {}, mod = {}, type = {}, itf = {} } = this.props
    ...
    return (
      <section className='PropertyList'>
        <div className='header clearfix'>
          ...
          <div className='toolbar'>
            <div className='btn-group'>
              ...
              <button type='button' className={`btn btn-secondary`} onClick={this.handleClickGenerateButton}>
                <GoCode className='fontsize-14' /> 生成
              </button>
            </div>
          </div>
        </div>
        ...
        <RModal when={this.state.createProperty} onClose={e => this.setState({ createProperty: false })} onResolve={this.handleCreatePropertySucceeded}>
          <PropertyForm title={`新建${label}属性`} scope={scope} repository={repository} mod={mod} type={type} itf={itf} />
        </RModal>
        <RModal when={!!this.state.createChildProperty} onClose={e => this.setState({ createChildProperty: false })} onResolve={this.handleCreatePropertySucceeded}>
          <PropertyForm title={`新建${label}属性`} scope={scope} repository={repository} mod={mod} type={type} itf={itf} parent={this.state.createChildProperty} />
        </RModal>
        <RModal when={this.state.importer} onClose={e => this.setState({ importer: false })} onResolve={this.handleCreatePropertySucceeded}>
          <Importer title={`导入${label}属性`} repository={repository} mod={mod} type={type} itf={itf} scope={scope} />
        </RModal>
        <RModal when={this.state.generate} onClose={e => this.setState({ generate: false })} onResolve={e => this.setState({ generate: false })}>
          <PropertyGenerate data={scopedProperties} scope={scope}/>
        </RModal>
      </section>
    )
  }
