import Json2Ts from '../../utils/jsonTots'
...
const treeToJson = (tree, tsStatus = false) => {
  const parse = (item, result) => {
    const required = tsStatus ? (item.required ? '' : '?') : ''
    let rule = (item.rule && !tsStatus) ? ('|' + item.rule) : ('' + required)
    let value = item.value
    var description = `${item.name}_description`;
    if(tsStatus && item.description) {
