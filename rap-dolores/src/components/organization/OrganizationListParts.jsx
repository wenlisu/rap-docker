...
export function handleSubmit (e, context, state) {
  e.preventDefault()
  let { onAddOrganization, onUpdateOrganization } = context
  let onAddOrUpdateOrganization = state.id ? onUpdateOrganization : onAddOrganization
  let organization = {
    ...state,
    // owner: auth.id, // 支持转移组织
    memberIds: (state.members || []).map(user => user.id)
  }
  let { owner, newOwner } = state
  if (newOwner && newOwner.id !== owner.id) organization.ownerId = newOwner.id
  onAddOrUpdateOrganization(organization, () => {
    let { store, rmodal } = context
