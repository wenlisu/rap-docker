...
export default {
  ...
  // 分类 type
  fetchTypeList ({ repositoryId = '', name = '' } = {}) {
    return fetch(`${serve}/type/list?repositoryId=${repositoryId}&name=${name}`, { ...CREDENTIALS })
      .then(res => res.json())
      .then(json => json.data)
  },
  fetchType (id) {
    return fetch(`${serve}/type/get?id=${id}`, { ...CREDENTIALS })
      .then(res => res.json())
      .then(json => json.data)
  },
  addType (type) {
    return fetch(`${serve}/type/create`, {
      ...CREDENTIALS,
      method: 'POST',
  ...
  updateType (type) {
    return fetch(`${serve}/type/update`, {
      ...CREDENTIALS,
      method: 'POST',
      body: JSON.stringify(type),
      headers: { 'Content-Type': 'application/json' }
  ...
  deleteType (id) {
    return fetch(`${serve}/type/remove?id=${id}`, { ...CREDENTIALS })
      .then(res => res.json())
      .then(json => json.data)
  },
  sortTypeList (ids) {
    return fetch(`${serve}/type/sort`, {
      ...CREDENTIALS,
      method: 'POST',
      body: JSON.stringify({ ids }),
      headers: { 'Content-Type': 'application/json' }
